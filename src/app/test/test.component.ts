...
export class RevueFormComponent {
  ...
  revueForm: FormGroup; // Formulaire pour ajouter une nouvelle revue
  ...
  constructor() { // Constructeur pour initialiser les formulaires
    this.revueForm = this.fb.group({ // Initialisation du formulaire pour ajouter une nouvelle revue
  ...
  async onSubmit() { // Méthode pour soumettre le formulaire de revue
    if (this.revueForm.valid) { // Vérifiez que le formulaire est valide avant de soumettre
      ...
      try {
        const formValue = this.revueForm.value; // Récupération des valeurs du formulaire de revue
        ...
        this.revueForm.reset(); // Réinitialiser le formulaire après soumission réussie
  ...
  async onUpdateSubmit() {
    if (this.editForm.valid && this.selectedRevueId) { // Vérifiez que le formulaire d'édition est valide et qu'un ID de revue est sélectionné
      ...
      try {
        ...
        const updatedData = { // Préparez les données à mettre à jour
          num_revue: Number(formValue.numRevue), // Assurez-vous que le numéro de revue est un nombre
          titre_revue: formValue.titre.trim(), // le trim() est utilisé pour enlever les espaces superflus
          date_sortie: formValue.dateSortie // Assurez-vous que la date est au format correct
        };

        const docRef = doc(this.firestore, 'revue', this.selectedRevueId); // Référence au document à mettre à jour
        await updateDoc(docRef, updatedData); // Mise à jour du document
        ...
        setTimeout(() => { // Temporisation pour laisser le temps à l'utilisateur de voir le succès
          this.cancelEdit(); // Réinitialiser l'état d'édition
        }, 2000); // 2 secondes
